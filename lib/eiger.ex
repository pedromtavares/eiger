defmodule Eiger do
  @moduledoc """
  Top-level domain for the periodic self-rehydrating cache.
  """
  ...
  def register_function(fun, key, ttl, refresh_interval)
    ...
    with {:error, :not_registered} <- get(key, 1000),
         computation = %{fun: fun, key: key, ttl: ttl, refresh_interval: refresh_interval} do
      GenServer.cast(Cache, {:register, computation})
    else
  ...
  def get(key, timeout \\ 30_000) when is_integer(timeout) and timeout > 0 do
    GenServer.call(Cache, {:get_or_wait, key, timeout}, timeout + 5000)
  end
end
